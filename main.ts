namespace SpriteKind {
    export const Object = SpriteKind.create()
    export const Item = SpriteKind.create()
    export const Light = SpriteKind.create()
}
controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        if (!(toolbar_selected)) {
            inventory.set_number(InventoryNumberAttribute.SelectedIndex, Math.max(inventory.get_number(InventoryNumberAttribute.SelectedIndex) - 8, 0))
        }
    }
    if (!(inventory_open)) {
        animation.runImageAnimation(
        Player1,
        [img`
            . . . . . . f f f f . . . . . . 
            . . . . f f e e e e f f . . . . 
            . . . f e e e f f e e e f . . . 
            . . f f f f f 2 2 f f f f f . . 
            . . f f e 2 e 2 2 e 2 e f f . . 
            . . f e 2 f 2 f f 2 f 2 e f . . 
            . . f f f 2 2 e e 2 2 f f f . . 
            . f f e f 2 f e e f 2 f e f f . 
            . f e e f f e e e e f e e e f . 
            . . f e e e e e e e e e e f . . 
            . . . f e e e e e e e e f . . . 
            . . e 4 f f f f f f f f 4 e . . 
            . . 4 d f 2 2 2 2 2 2 f d 4 . . 
            . . 4 4 f 4 4 4 4 4 4 f 4 4 . . 
            . . . . . f f f f f f . . . . . 
            . . . . . f f . . f f . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f . . . . . . 
            . . . . f f e e e e f f . . . . 
            . . . f e e e f f e e e f . . . 
            . . . f f f f 2 2 f f f f . . . 
            . . f f e 2 e 2 2 e 2 e f f . . 
            . . f e 2 f 2 f f f 2 f e f . . 
            . . f f f 2 f e e 2 2 f f f . . 
            . . f e 2 f f e e 2 f e e f . . 
            . f f e f f e e e f e e e f f . 
            . f f e e e e e e e e e e f f . 
            . . . f e e e e e e e e f . . . 
            . . . e f f f f f f f f 4 e . . 
            . . . 4 f 2 2 2 2 2 e d d 4 . . 
            . . . e f f f f f f e e 4 . . . 
            . . . . f f f . . . . . . . . . 
            `,img`
            . . . . . . f f f f . . . . . . 
            . . . . f f e e e e f f . . . . 
            . . . f e e e f f e e e f . . . 
            . . f f f f f 2 2 f f f f f . . 
            . . f f e 2 e 2 2 e 2 e f f . . 
            . . f e 2 f 2 f f 2 f 2 e f . . 
            . . f f f 2 2 e e 2 2 f f f . . 
            . f f e f 2 f e e f 2 f e f f . 
            . f e e f f e e e e f e e e f . 
            . . f e e e e e e e e e e f . . 
            . . . f e e e e e e e e f . . . 
            . . e 4 f f f f f f f f 4 e . . 
            . . 4 d f 2 2 2 2 2 2 f d 4 . . 
            . . 4 4 f 4 4 4 4 4 4 f 4 4 . . 
            . . . . . f f f f f f . . . . . 
            . . . . . f f . . f f . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f . . . . . . 
            . . . . f f e e e e f f . . . . 
            . . . f e e e f f e e e f . . . 
            . . . f f f f 2 2 f f f f . . . 
            . . f f e 2 e 2 2 e 2 e f f . . 
            . . f e f 2 f f f 2 f 2 e f . . 
            . . f f f 2 2 e e f 2 f f f . . 
            . . f e e f 2 e e f f 2 e f . . 
            . f f e e e f e e e f f e f f . 
            . f f e e e e e e e e e e f f . 
            . . . f e e e e e e e e f . . . 
            . . e 4 f f f f f f f f e . . . 
            . . 4 d d e 2 2 2 2 2 f 4 . . . 
            . . . 4 e e f f f f f f e . . . 
            . . . . . . . . . f f f . . . . 
            `],
        100,
        true
        )
        Player1.setVelocity(0, -50)
    }
})
scene.onHitWall(SpriteKind.Player, function (sprite, location) {
    if (toolbar.get_items()[toolbar.get_number(ToolbarNumberAttribute.SelectedIndex)] == Axe._pickRandom() && tiles.tileAtLocationEquals(location, sprites.castle.saplingPine)) {
        tiles.setWallAt(location, false)
        tiles.setTileAt(location, assets.tile`baseTransparency16`)
        Wood_Drop = randint(1, 2)
        for (let item of Wood) {
            if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
                item.set_text(ItemTextAttribute.Tooltip, "" + (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) + Wood_Drop))
            } else {
                item.set_text(ItemTextAttribute.Tooltip, "" + Wood_Drop)
                inventory.get_items().push(item)
            }
            Total_Wood = Total_Wood + Wood_Drop
        }
    }
    if (toolbar.get_items()[toolbar.get_number(ToolbarNumberAttribute.SelectedIndex)] == Pickaxe._pickRandom() && tiles.tileAtLocationEquals(location, sprites.castle.rock0)) {
        tiles.setWallAt(location, false)
        tiles.setTileAt(location, assets.tile`baseTransparency16`)
        Stone_Drop = randint(1, 2)
        for (let item of Stone) {
            if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
                item.set_text(ItemTextAttribute.Tooltip, "" + (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) + Stone_Drop))
            } else {
                item.set_text(ItemTextAttribute.Tooltip, "" + Stone_Drop)
                inventory.get_items().push(item)
            }
            Total_Stone = Total_Stone + Stone_Drop
        }
    }
    if (toolbar.get_items()[toolbar.get_number(ToolbarNumberAttribute.SelectedIndex)] == Stone_Pickaxe._pickRandom() && (tiles.tileAtLocationEquals(location, sprites.castle.rock0) || tiles.tileAtLocationEquals(location, assets.tile`myTile1`))) {
        tiles.setWallAt(location, false)
        tiles.setTileAt(location, assets.tile`baseTransparency16`)
        if (tiles.tileAtLocationEquals(location, assets.tile`myTile1`)) {
            Iron_Drop = randint(1, 2)
            for (let item of Iron) {
                if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
                    item.set_text(ItemTextAttribute.Tooltip, "" + (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) + Iron_Drop))
                } else {
                    item.set_text(ItemTextAttribute.Tooltip, "" + Iron_Drop)
                    inventory.get_items().push(item)
                }
                Total_Iron = Total_Iron + Iron_Drop
            }
        } else {
            Stone_Drop = randint(1, 2)
            for (let item of Stone) {
                if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
                    item.set_text(ItemTextAttribute.Tooltip, "" + (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) + Stone_Drop))
                } else {
                    item.set_text(ItemTextAttribute.Tooltip, "" + Stone_Drop)
                    inventory.get_items().push(item)
                }
                Total_Stone = Total_Stone + Stone_Drop
            }
        }
    }
})
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        switch_between_toolbar_and_inventory()
    } else {
        toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, toolbar.get_number(ToolbarNumberAttribute.SelectedIndex) + 1)
        if (toolbar.get_number(ToolbarNumberAttribute.SelectedIndex) + 1 > toolbar.get_number(ToolbarNumberAttribute.MaxItems)) {
            toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, 0)
        }
    }
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        if (toolbar_selected) {
            if (toolbar.get_items()[toolbar.get_number(ToolbarNumberAttribute.SelectedIndex)] && inventory.get_items().length < inventory.get_number(InventoryNumberAttribute.MaxItems)) {
                inventory.get_items().push(toolbar.get_items().removeAt(toolbar.get_number(ToolbarNumberAttribute.SelectedIndex)))
            }
        } else {
            if (inventory.get_items()[inventory.get_number(InventoryNumberAttribute.SelectedIndex)] && toolbar.get_items().length < toolbar.get_number(ToolbarNumberAttribute.MaxItems)) {
                toolbar.get_items().push(inventory.get_items().removeAt(inventory.get_number(InventoryNumberAttribute.SelectedIndex)))
            }
        }
        switch_between_toolbar_and_inventory()
    }
    if (Player1.tileKindAt(TileDirection.Left, assets.tile`myTile0`) || (Player1.tileKindAt(TileDirection.Right, assets.tile`myTile0`) || (Player1.tileKindAt(TileDirection.Top, assets.tile`myTile0`) || Player1.tileKindAt(TileDirection.Bottom, assets.tile`myTile0`)))) {
        game.splash("Stick = 1 Pickaxe = 2 Stone Pickaxe = 3")
        game.splash("What would you like to craft?")
        Craft = game.askForNumber("")
        if (Craft == 1 && Total_Wood > 0) {
            CraftingStick()
        }
        if (Craft == 2 && Total_Wood > 2 && Total_Sticks > 1) {
            CraftingPickaxe()
        }
        if (Craft == 3 && Total_Stone > 2 && Total_Sticks > 1) {
            CraftingPickaxe2()
        }
    }
    toolbar.update()
    inventory.update()
})
controller.down.onEvent(ControllerButtonEvent.Released, function () {
    Player1.setVelocity(0, 0)
    animation.stopAnimation(animation.AnimationTypes.All, Player1)
})
function CraftingStick () {
    for (let item of Stick) {
        if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
            item.set_text(ItemTextAttribute.Tooltip, "" + (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) + 1))
        } else {
            item.set_text(ItemTextAttribute.Tooltip, "1")
            inventory.get_items().push(item)
        }
        Total_Wood += -1
        Total_Sticks += 1
    }
    for (let item of Wood) {
        item.set_text(ItemTextAttribute.Tooltip, "" + Total_Wood)
    }
    inventory.update()
}
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        if (toolbar_selected) {
            toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, Math.max(toolbar.get_number(ToolbarNumberAttribute.SelectedIndex) - 1, 0))
        } else {
            inventory.set_number(InventoryNumberAttribute.SelectedIndex, Math.max(inventory.get_number(InventoryNumberAttribute.SelectedIndex) - 1, 0))
        }
    }
    if (!(inventory_open)) {
        animation.runImageAnimation(
        Player1,
        [img`
            . . . . f f f f f f . . . . . . 
            . . . f 2 f e e e e f f . . . . 
            . . f 2 2 2 f e e e e f f . . . 
            . . f e e e e f f e e e f . . . 
            . f e 2 2 2 2 e e f f f f . . . 
            . f 2 e f f f f 2 2 2 e f . . . 
            . f f f e e e f f f f f f f . . 
            . f e e 4 4 f b e 4 4 e f f . . 
            . . f e d d f 1 4 d 4 e e f . . 
            . . . f d d d d 4 e e e f . . . 
            . . . f e 4 4 4 e e f f . . . . 
            . . . f 2 2 2 e d d 4 . . . . . 
            . . . f 2 2 2 e d d e . . . . . 
            . . . f 5 5 4 f e e f . . . . . 
            . . . . f f f f f f . . . . . . 
            . . . . . . f f f . . . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . f f f f f f . . . . . . 
            . . . f 2 f e e e e f f . . . . 
            . . f 2 2 2 f e e e e f f . . . 
            . . f e e e e f f e e e f . . . 
            . f e 2 2 2 2 e e f f f f . . . 
            . f 2 e f f f f 2 2 2 e f . . . 
            . f f f e e e f f f f f f f . . 
            . f e e 4 4 f b e 4 4 e f f . . 
            . . f e d d f 1 4 d 4 e e f . . 
            . . . f d d d e e e e e f . . . 
            . . . f e 4 e d d 4 f . . . . . 
            . . . f 2 2 e d d e f . . . . . 
            . . f f 5 5 f e e f f f . . . . 
            . . f f f f f f f f f f . . . . 
            . . . f f f . . . f f . . . . . 
            `,img`
            . . . . f f f f f f . . . . . . 
            . . . f 2 f e e e e f f . . . . 
            . . f 2 2 2 f e e e e f f . . . 
            . . f e e e e f f e e e f . . . 
            . f e 2 2 2 2 e e f f f f . . . 
            . f 2 e f f f f 2 2 2 e f . . . 
            . f f f e e e f f f f f f f . . 
            . f e e 4 4 f b e 4 4 e f f . . 
            . . f e d d f 1 4 d 4 e e f . . 
            . . . f d d d d 4 e e e f . . . 
            . . . f e 4 4 4 e e f f . . . . 
            . . . f 2 2 2 e d d 4 . . . . . 
            . . . f 2 2 2 e d d e . . . . . 
            . . . f 5 5 4 f e e f . . . . . 
            . . . . f f f f f f . . . . . . 
            . . . . . . f f f . . . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . f f f f f f . . . . . . 
            . . . f 2 f e e e e f f . . . . 
            . . f 2 2 2 f e e e e f f . . . 
            . . f e e e e f f e e e f . . . 
            . f e 2 2 2 2 e e f f f f . . . 
            . f 2 e f f f f 2 2 2 e f . . . 
            . f f f e e e f f f f f f f . . 
            . f e e 4 4 f b e 4 4 e f f . . 
            . . f e d d f 1 4 d 4 e e f . . 
            . . . f d d d d 4 e e e f . . . 
            . . . f e 4 4 4 e d d 4 . . . . 
            . . . f 2 2 2 2 e d d e . . . . 
            . . f f 5 5 4 4 f e e f . . . . 
            . . f f f f f f f f f f . . . . 
            . . . f f f . . . f f . . . . . 
            `],
        100,
        true
        )
        Player1.setVelocity(-50, 0)
    }
})
function CraftingPickaxe () {
    for (let item of Pickaxe) {
        if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
            game.splash("You already have a pickaxe!")
        } else {
            inventory.get_items().push(item)
        }
        Total_Wood += -3
        Total_Sticks += -2
    }
    for (let item of Wood) {
        item.set_text(ItemTextAttribute.Tooltip, "" + Total_Wood)
    }
    for (let item of Stick) {
        item.set_text(ItemTextAttribute.Tooltip, "" + Total_Sticks)
    }
    inventory.update()
}
controller.right.onEvent(ControllerButtonEvent.Released, function () {
    Player1.setVelocity(0, 0)
    animation.stopAnimation(animation.AnimationTypes.All, Player1)
})
controller.left.onEvent(ControllerButtonEvent.Released, function () {
    Player1.setVelocity(0, 0)
    animation.stopAnimation(animation.AnimationTypes.All, Player1)
})
function switch_between_toolbar_and_inventory () {
    toolbar_selected = !(toolbar_selected)
    if (toolbar_selected) {
        inventory.set_number(InventoryNumberAttribute.SelectedIndex, -1)
        toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, 0)
    } else {
        toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, -1)
        inventory.set_number(InventoryNumberAttribute.SelectedIndex, 0)
    }
}
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        if (toolbar_selected) {
            toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, Math.min(toolbar.get_number(ToolbarNumberAttribute.SelectedIndex) + 1, toolbar.get_number(ToolbarNumberAttribute.MaxItems) - 1))
        } else {
            inventory.set_number(InventoryNumberAttribute.SelectedIndex, Math.min(inventory.get_number(InventoryNumberAttribute.SelectedIndex) + 1, inventory.get_items().length - 1))
        }
    }
    if (!(inventory_open)) {
        animation.runImageAnimation(
        Player1,
        [img`
            . . . . . . f f f f f f . . . . 
            . . . . f f e e e e f 2 f . . . 
            . . . f f e e e e f 2 2 2 f . . 
            . . . f e e e f f e e e e f . . 
            . . . f f f f e e 2 2 2 2 e f . 
            . . . f e 2 2 2 f f f f e 2 f . 
            . . f f f f f f f e e e f f f . 
            . . f f e 4 4 e b f 4 4 e e f . 
            . . f e e 4 d 4 1 f d d e f . . 
            . . . f e e e 4 d d d d f . . . 
            . . . . f f e e 4 4 4 e f . . . 
            . . . . . 4 d d e 2 2 2 f . . . 
            . . . . . e d d e 2 2 2 f . . . 
            . . . . . f e e f 4 5 5 f . . . 
            . . . . . . f f f f f f . . . . 
            . . . . . . . f f f . . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f f f . . . . 
            . . . . f f e e e e f 2 f . . . 
            . . . f f e e e e f 2 2 2 f . . 
            . . . f e e e f f e e e e f . . 
            . . . f f f f e e 2 2 2 2 e f . 
            . . . f e 2 2 2 f f f f e 2 f . 
            . . f f f f f f f e e e f f f . 
            . . f f e 4 4 e b f 4 4 e e f . 
            . . f e e 4 d 4 1 f d d e f . . 
            . . . f e e e e e d d d f . . . 
            . . . . . f 4 d d e 4 e f . . . 
            . . . . . f e d d e 2 2 f . . . 
            . . . . f f f e e f 5 5 f f . . 
            . . . . f f f f f f f f f f . . 
            . . . . . f f . . . f f f . . . 
            `,img`
            . . . . . . f f f f f f . . . . 
            . . . . f f e e e e f 2 f . . . 
            . . . f f e e e e f 2 2 2 f . . 
            . . . f e e e f f e e e e f . . 
            . . . f f f f e e 2 2 2 2 e f . 
            . . . f e 2 2 2 f f f f e 2 f . 
            . . f f f f f f f e e e f f f . 
            . . f f e 4 4 e b f 4 4 e e f . 
            . . f e e 4 d 4 1 f d d e f . . 
            . . . f e e e 4 d d d d f . . . 
            . . . . f f e e 4 4 4 e f . . . 
            . . . . . 4 d d e 2 2 2 f . . . 
            . . . . . e d d e 2 2 2 f . . . 
            . . . . . f e e f 4 5 5 f . . . 
            . . . . . . f f f f f f . . . . 
            . . . . . . . f f f . . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f f f . . . . 
            . . . . f f e e e e f 2 f . . . 
            . . . f f e e e e f 2 2 2 f . . 
            . . . f e e e f f e e e e f . . 
            . . . f f f f e e 2 2 2 2 e f . 
            . . . f e 2 2 2 f f f f e 2 f . 
            . . f f f f f f f e e e f f f . 
            . . f f e 4 4 e b f 4 4 e e f . 
            . . f e e 4 d 4 1 f d d e f . . 
            . . . f e e e 4 d d d d f . . . 
            . . . . 4 d d e 4 4 4 e f . . . 
            . . . . e d d e 2 2 2 2 f . . . 
            . . . . f e e f 4 4 5 5 f f . . 
            . . . . f f f f f f f f f f . . 
            . . . . . f f . . . f f f . . . 
            `],
        100,
        true
        )
        Player1.setVelocity(50, 0)
    }
})
controller.up.onEvent(ControllerButtonEvent.Released, function () {
    Player1.setVelocity(0, 0)
    animation.stopAnimation(animation.AnimationTypes.All, Player1)
})
controller.down.onEvent(ControllerButtonEvent.Pressed, function () {
    if (inventory_open) {
        if (!(toolbar_selected)) {
            inventory.set_number(InventoryNumberAttribute.SelectedIndex, Math.min(inventory.get_number(InventoryNumberAttribute.SelectedIndex) + 8, inventory.get_items().length - 1))
        }
    }
    if (!(inventory_open)) {
        animation.runImageAnimation(
        Player1,
        [img`
            . . . . . . f f f f . . . . . . 
            . . . . f f f 2 2 f f f . . . . 
            . . . f f f 2 2 2 2 f f f . . . 
            . . f f f e e e e e e f f f . . 
            . . f f e 2 2 2 2 2 2 e e f . . 
            . . f e 2 f f f f f f 2 e f . . 
            . . f f f f e e e e f f f f . . 
            . f f e f b f 4 4 f b f e f f . 
            . f e e 4 1 f d d f 1 4 e e f . 
            . . f e e d d d d d d e e f . . 
            . . . f e e 4 4 4 4 e e f . . . 
            . . e 4 f 2 2 2 2 2 2 f 4 e . . 
            . . 4 d f 2 2 2 2 2 2 f d 4 . . 
            . . 4 4 f 4 4 5 5 4 4 f 4 4 . . 
            . . . . . f f f f f f . . . . . 
            . . . . . f f . . f f . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f . . . . . . 
            . . . . f f f 2 2 f f f . . . . 
            . . . f f f 2 2 2 2 f f f . . . 
            . . f f f e e e e e e f f f . . 
            . . f f e 2 2 2 2 2 2 e e f . . 
            . f f e 2 f f f f f f 2 e f f . 
            . f f f f f e e e e f f f f f . 
            . . f e f b f 4 4 f b f e f . . 
            . . f e 4 1 f d d f 1 4 e f . . 
            . . . f e 4 d d d d 4 e f e . . 
            . . f e f 2 2 2 2 e d d 4 e . . 
            . . e 4 f 2 2 2 2 e d d e . . . 
            . . . . f 4 4 5 5 f e e . . . . 
            . . . . f f f f f f f . . . . . 
            . . . . f f f . . . . . . . . . 
            `,img`
            . . . . . . f f f f . . . . . . 
            . . . . f f f 2 2 f f f . . . . 
            . . . f f f 2 2 2 2 f f f . . . 
            . . f f f e e e e e e f f f . . 
            . . f f e 2 2 2 2 2 2 e e f . . 
            . . f e 2 f f f f f f 2 e f . . 
            . . f f f f e e e e f f f f . . 
            . f f e f b f 4 4 f b f e f f . 
            . f e e 4 1 f d d f 1 4 e e f . 
            . . f e e d d d d d d e e f . . 
            . . . f e e 4 4 4 4 e e f . . . 
            . . e 4 f 2 2 2 2 2 2 f 4 e . . 
            . . 4 d f 2 2 2 2 2 2 f d 4 . . 
            . . 4 4 f 4 4 5 5 4 4 f 4 4 . . 
            . . . . . f f f f f f . . . . . 
            . . . . . f f . . f f . . . . . 
            `,img`
            . . . . . . . . . . . . . . . . 
            . . . . . . f f f f . . . . . . 
            . . . . f f f 2 2 f f f . . . . 
            . . . f f f 2 2 2 2 f f f . . . 
            . . f f f e e e e e e f f f . . 
            . . f e e 2 2 2 2 2 2 e f f . . 
            . f f e 2 f f f f f f 2 e f f . 
            . f f f f f e e e e f f f f f . 
            . . f e f b f 4 4 f b f e f . . 
            . . f e 4 1 f d d f 1 4 e f . . 
            . . e f e 4 d d d d 4 e f . . . 
            . . e 4 d d e 2 2 2 2 f e f . . 
            . . . e d d e 2 2 2 2 f 4 e . . 
            . . . . e e f 5 5 4 4 f . . . . 
            . . . . . f f f f f f f . . . . 
            . . . . . . . . . f f f . . . . 
            `],
        100,
        true
        )
        Player1.setVelocity(0, 50)
    }
})
controller.menu.onEvent(ControllerButtonEvent.Pressed, function () {
    inventory_open = !(inventory_open)
    inventory.set_number(InventoryNumberAttribute.SelectedIndex, inventory.get_number(InventoryNumberAttribute.SelectedIndex))
    inventory.setFlag(SpriteFlag.Invisible, !(inventory_open))
    if (!(inventory_open)) {
        inventory.set_number(InventoryNumberAttribute.SelectedIndex, -1)
        toolbar.set_number(ToolbarNumberAttribute.SelectedIndex, 0)
        toolbar_selected = true
    }
})
function create_toolbar_and_inventory () {
    toolbar = Inventory.create_toolbar([], 3)
    toolbar.left = 4
    toolbar.bottom = scene.screenHeight() - 4
    toolbar.z = 100
    toolbar.setFlag(SpriteFlag.RelativeToCamera, true)
    inventory = Inventory.create_inventory([], 32)
    inventory.set_number(InventoryNumberAttribute.SelectedIndex, -1)
    inventory.left = 4
    inventory.top = 4
    inventory.z = 100
    inventory.setFlag(SpriteFlag.RelativeToCamera, true)
    inventory_open = false
    inventory.setFlag(SpriteFlag.Invisible, !(inventory_open))
    toolbar_selected = true
}
function CraftingPickaxe2 () {
    for (let item of Stone_Pickaxe) {
        if (parseFloat(item.get_text(ItemTextAttribute.Tooltip)) > 0) {
            game.splash("You already have a stone pickaxe!")
        } else {
            inventory.get_items().push(item)
        }
        Total_Stone += -3
        Total_Sticks += -2
    }
    for (let item of Stone) {
        item.set_text(ItemTextAttribute.Tooltip, "" + Total_Stone)
    }
    for (let item of Stick) {
        item.set_text(ItemTextAttribute.Tooltip, "" + Total_Sticks)
    }
    inventory.update()
}
let Craft = 0
let Total_Iron = 0
let Iron_Drop = 0
let Total_Stone = 0
let Stone_Drop = 0
let Total_Wood = 0
let Wood_Drop = 0
let inventory: Inventory.Inventory = null
let toolbar_selected = false
let inventory_open = false
let Total_Sticks = 0
let Stone: Inventory.Item[] = []
let Iron: Inventory.Item[] = []
let Stick: Inventory.Item[] = []
let Stone_Pickaxe: Inventory.Item[] = []
let Pickaxe: Inventory.Item[] = []
let Wood: Inventory.Item[] = []
let toolbar: Inventory.Toolbar = null
let Axe: Inventory.Item[] = []
let Player1: Sprite = null
scene.setBackgroundImage(img`
    
    `)
tiles.setCurrentTilemap(tilemap`Start`)
Player1 = sprites.create(img`
    . . . . . . f f f f . . . . . . 
    . . . . f f f 2 2 f f f . . . . 
    . . . f f f 2 2 2 2 f f f . . . 
    . . f f f e e e e e e f f f . . 
    . . f f e 2 2 2 2 2 2 e e f . . 
    . . f e 2 f f f f f f 2 e f . . 
    . . f f f f e e e e f f f f . . 
    . f f e f b f 4 4 f b f e f f . 
    . f e e 4 1 f d d f 1 4 e e f . 
    . . f e e d d d d d d e e f . . 
    . . . f e e 4 4 4 4 e e f . . . 
    . . e 4 f 2 2 2 2 2 2 f 4 e . . 
    . . 4 d f 2 2 2 2 2 2 f d 4 . . 
    . . 4 4 f 4 4 5 5 4 4 f 4 4 . . 
    . . . . . f f f f f f . . . . . 
    . . . . . f f . . f f . . . . . 
    `, SpriteKind.Player)
tiles.placeOnRandomTile(Player1, assets.tile`baseTransparency16`)
scene.cameraFollowSprite(Player1)
create_toolbar_and_inventory()
Axe = [Inventory.create_item("Axe", assets.image`Axe`)]
toolbar.get_items().push(Axe._pickRandom())
toolbar.update()
Wood = [Inventory.create_item("\"Wood\"", assets.image`Wood`)]
Pickaxe = [Inventory.create_item("Pickaxe", assets.image`myImage`)]
Stone_Pickaxe = [Inventory.create_item("Stone Pickaxe", assets.image`myImage0`)]
Stick = [Inventory.create_item("Stick", assets.image`Axe1`)]
Iron = [Inventory.create_item("Iron", assets.image`myImage1`)]
Stone = [Inventory.create_item("Stone", assets.image`myImage2`)]
Total_Sticks = 0
